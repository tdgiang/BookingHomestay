...
export function RoomsContent({ initial }: { initial: InitialData | null }) {
  const searchParams = useSearchParams();
  const router = useRouter();

  const [rooms, setRooms] = useState<Room[]>(initial?.items ?? []);
  ...
  // Only refetch when user changes filters (not on initial SSR-hydrated render)
